...
| WAX-18 | The MCP shim's auto-compact trigger was a flat 64 MB over the stamp, below one delta append (~0.5 arena-equivalents, ~67 MB on the 139 MB reference arena), so every one-file edit spawned a full `--compact` rebuild (~20 min). Separately, `refresh.log` was unstamped and captured detached stdout only, so CLI-run rebuilds left no explanation; and the shim spawned `--compact` without probing the arena lease, racing a live semantic `serve` into `IndexFailure: arena lease unavailable`. | Fixed: `wax_needs_compact` now derives its threshold from the CLI's own band (`stamp*(1+maxGrowth) + max(allowanceBytes, stamp*scale)`, same env vars as `WaxDeltaPolicy.evaluate`); every index run appends timestamped `DeltaDecision`/`WaxCompact`/failure records to `.cckit/index-runs.jsonl` (CLI runs included); and the shim probes `.cckit/repo.wax.lock` and defers with `wax_lease_held` instead of spawning into a held arena. |
